// TODO make the objects moving continuously
/*
    ...
    fileInput.addEventListener('change', function (e) {
        ...
        reader.onload = function (e) {
            ...
            if (data[0] !== "ply") {
                ...
                //pulsePreparation(); //todo in case of errors
    ...
    window.onkeypress = function (event) {
        ...
        switch (key) {
            ...
            case 'z': // Translate your wireframe in the positive z direction
                ...
                break;
            case 'p':
                if (pulseMode === true) {pulseMode = false;}
                else {pulseMode = true;}
                break;
            case 'r':
                theta += 2;
...
function render() {
    ...
    /*
        ...
        //var rotMatrixY = rotateY(alpha);
        //var rotMatrix2 = rotateX(45);



        if (pulseMode) {
            var translateMatrix = translate(pulsedMesh[i][0], pulsedMesh[i][1], pulsedMesh[i][2]);
        } else {
...
function newellMethod(a, b, c) {
    ...
    normals.push(vec3(nx, ny, nz));  // contains the a vector per triangle, so (points.length / 3) vectors
}

// I think that this function should modify the points array
function pulsePreparation() {
    //for each triangle
    for (var i = 0; i < (points.length) / 3; i++) { //(points.length)/3 is the number of triangles. At the ith iteration we refer to the ith triangle.
        ...
        for (var j = 0; j < 3; j++) { // for each vertex we sum the component of the normal
            tpx = points[i * 3 + j][0] + 0.1 * n[i][0];
            tpy = points[i * 3 + j][1] + 0.1 * n[i][1];
            tpz = points[i * 3 + j][2] + 0.1 * n[i][2];

            pulsedMesh.push(vec3(tpx, tpy, tpz)); // we add the new vertices
            /*
            for (var k = 0; k < 3; k++) { // for each coordinate
              ...
              points[i*3 + j][k] = points[i*3 + j][k] + 0.5*n[k];
            } */
        }
